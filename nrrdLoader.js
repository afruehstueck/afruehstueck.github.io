...
//this loader is a modified version of the NRRD loader in three.js
// the parser is largely inspired from the XTK NRRD parser : https://github.com/xtk/X
var NRRDLoader = {
	parse: function ( data ) {
		...
		function parseHeader( header ) {
			...
			for ( _i = 0, _len = lines.length; _i < _len; _i++ ) {
				...
				if ( l.match( /NRRD\d+/ ) ) {
					...
					field = m[ 1 ].trim();
					data = m[ 2 ].trim();
					fn = NRRDLoader.fieldFunctions[ field ];
					if ( fn ) {
			...
			if ( !headerObject.vectors ) {
				...
				headerObject.vectors = [ vec3.fromValues( 1, 0, 0 ), vec3.fromValues( 0, 1, 0 ), vec3.fromValues( 0, 0, 1 ) ];
				...
				if ( headerObject.spacings ) {
					...
					for ( i = 0; i <= 2; i++ ) {
						...
						if ( !isNaN( headerObject.spacings[ i ] ) ) {
							...
							vec3.multiply( headerObject.vectors[ i ], headerObject.vectors[ i ], headerObject.spacings[ i ] );
		...
		// spacing
		var spacingX = vec3.length( ( vec3.fromValues( headerObject.vectors[ 0 ][ 0 ], headerObject.vectors[ 0 ][ 1 ],
			headerObject.vectors[ 0 ][ 2 ] ) ) );
		var spacingY = vec3.length( ( vec3.fromValues( headerObject.vectors[ 1 ][ 0 ], headerObject.vectors[ 1 ][ 1 ],
		...
		if ( !headerObject.vectors ) {
			...
			volume.matrix = mat4.fromValues( _spaceX, 0, 0, 0,
				0, _spaceY, 0, 0,
				0, 0, _spaceZ, 0,
				0, 0, 0, 1 );
			...
			volume.matrix = mat4.fromValues( _spaceX * v[ 0 ][ 0 ], _spaceX * v[ 1 ][ 0 ], _spaceX * v[ 2 ][ 0 ], 0,
		...
		volume.inverseMatrix = mat4.create();
		mat4.invert( volume.inverseMatrix, volume.matrix );
		//TODO what is this //volume.RASDimensions = ( vec3.fromValues( volume.xLength, volume.yLength, volume.zLength ) ).applyMatrix4( volume.matrix ).round().toArray().map( Math.abs );

		// .. and set the default threshold
		// only if the threshold was not already set
		if ( volume.lowerThreshold === -Infinity ) {

			volume.lowerThreshold = range.min;

		}
		if ( volume.upperThreshold === Infinity ) {

			volume.upperThreshold = range.max;
